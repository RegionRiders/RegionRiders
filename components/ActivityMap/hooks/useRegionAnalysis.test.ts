...
jest.mock('@/lib/utils/regionVisitAnalyzer', () => ({
    analyzeRegionVisitsAsync: jest.fn(() => Promise.resolve(new Map([
        ['region1', {
            regionId: 'region1',
...
describe('useRegionAnalysis', () => {
    ...
    describe('initial state', () => {
        it('should start with empty visitData', () => {
            const { result } = renderHook(() =>
                useRegionAnalysis(mockTracks, mockRegions)
            );
    ...
    describe('analysis', () => {
        ...
        it('should handle empty tracks', () => {
            const { result } = renderHook(() =>
                useRegionAnalysis(new Map(), mockRegions)
            );
