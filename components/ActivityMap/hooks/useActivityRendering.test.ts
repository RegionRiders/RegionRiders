...
describe('useActivityRendering', () => {
    ...
    it('should not call drawActivities when map is null', () => {
        renderHook(() =>
            useActivityRendering(null, mockTracks, true, 'heatmap')
        );

        expect(drawActivities).not.toHaveBeenCalled();
        expect(logger.info).toHaveBeenCalledWith(
            expect.stringContaining('Skipping render')
        );
    });

    it('should not call drawActivities when showActivities is false', () => {
        renderHook(() =>
            useActivityRendering(mockMap, mockTracks, false, 'heatmap')
        );
    ...
    it('should not call drawActivities when tracks is empty', () => {
        ...
        renderHook(() =>
            useActivityRendering(mockMap, emptyTracks, true, 'heatmap')
        );
    ...
    it('should handle multiple tracks correctly', () => {
        ...
        renderHook(() =>
            useActivityRendering(mockMap, multipleTracks, true, 'heatmap')
        );
